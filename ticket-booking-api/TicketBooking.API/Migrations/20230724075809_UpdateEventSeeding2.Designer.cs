// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using TicketBooking.API.EF;

#nullable disable

namespace TicketBooking.API.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20230724075809_UpdateEventSeeding2")]
    partial class UpdateEventSeeding2
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "7.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("TicketBooking.API.Models.Category", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("nvarchar(450)")
                        .HasDefaultValue("ce0d2b81-42e8-4c3e-a645-9806c61662bd");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValue(new DateTime(2023, 7, 24, 14, 58, 8, 905, DateTimeKind.Local).AddTicks(751));

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.ToTable("Category", (string)null);

                    b.HasData(
                        new
                        {
                            Id = "c1",
                            IsDeleted = false,
                            Name = "Theater & Play"
                        },
                        new
                        {
                            Id = "c2",
                            IsDeleted = false,
                            Name = "Live music"
                        },
                        new
                        {
                            Id = "c3",
                            IsDeleted = false,
                            Name = "Art & Culture"
                        },
                        new
                        {
                            Id = "c4",
                            IsDeleted = false,
                            Name = "Nightlife"
                        },
                        new
                        {
                            Id = "c5",
                            IsDeleted = false,
                            Name = "Outdoor"
                        },
                        new
                        {
                            Id = "c6",
                            IsDeleted = false,
                            Name = "Conference"
                        },
                        new
                        {
                            Id = "c7",
                            IsDeleted = false,
                            Name = "Seminar"
                        },
                        new
                        {
                            Id = "c8",
                            IsDeleted = false,
                            Name = "Exhibitions"
                        },
                        new
                        {
                            Id = "c9",
                            IsDeleted = false,
                            Name = "Meetups"
                        },
                        new
                        {
                            Id = "c10",
                            IsDeleted = false,
                            Name = "Sports"
                        },
                        new
                        {
                            Id = "c11",
                            IsDeleted = false,
                            Name = "Comunity"
                        },
                        new
                        {
                            Id = "c12",
                            IsDeleted = false,
                            Name = "Attractions"
                        });
                });

            modelBuilder.Entity("TicketBooking.API.Models.Event", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("nvarchar(450)")
                        .HasDefaultValue("e20a63a1-06e2-4421-bcb8-35bff0cfea7d");

                    b.Property<string>("City")
                        .IsUnicode(true)
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValue(new DateTime(2023, 7, 24, 14, 58, 8, 900, DateTimeKind.Local).AddTicks(3949));

                    b.Property<DateTime>("Date")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValue(new DateTime(2023, 8, 11, 21, 15, 0, 0, DateTimeKind.Unspecified));

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Duration")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Image")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("nvarchar(max)")
                        .HasDefaultValue("");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<string>("Location")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("MinPrice")
                        .HasColumnType("int");

                    b.Property<int>("Status")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(2);

                    b.Property<string>("Title")
                        .IsRequired()
                        .IsUnicode(true)
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.ToTable("Event", (string)null);

                    b.HasData(
                        new
                        {
                            Id = "e1",
                            City = "",
                            Date = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Duration = "7:30 PM - 9:30 PM",
                            Image = "https://images.tkbcdn.com/1/1560/600/Upload/eventcover/2023/07/18/AA2A4B.jpg",
                            IsDeleted = false,
                            Location = "The Nest, Thôn Măng Lin, Phường 7, Đà Lạt",
                            MinPrice = 350000,
                            Status = 0,
                            Title = "[MÂY LANG THANG] LIVESHOW PHAN MẠNH QUỲNH KM TRẦN MINH DŨNG"
                        },
                        new
                        {
                            Id = "e2",
                            City = "",
                            Date = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Duration = "10:00 PM - 11:30 PM",
                            Image = "https://images.tkbcdn.com/1/1560/600/Upload/eventcover/2023/07/18/238BEE.jpg",
                            IsDeleted = false,
                            Location = "The Nest, Thôn Măng Lin, Phường 7, Đà Lạt",
                            MinPrice = 450000,
                            Status = 0,
                            Title = "[MÂY LANG THANG] PHƯƠNG UYÊN"
                        },
                        new
                        {
                            Id = "e3",
                            City = "Hà Nội",
                            Date = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Duration = "7:30 PM - 9:30 PM",
                            Image = "https://images.tkbcdn.com/1/1560/600/Upload/eventcover/2023/07/13/23D46F.jpg",
                            IsDeleted = false,
                            Location = "Rạp Đại Nam, 89 Phố Huế, Ngô Thì Nhậm, Hai Bà Trưng, Hà Nội",
                            MinPrice = 5000000,
                            Status = 0,
                            Title = "[MÂY LANG THANG HÀ NỘI] LIVESHOW TUẤN NGỌC"
                        },
                        new
                        {
                            Id = "e4",
                            City = "Cần Thơ",
                            Date = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Duration = "7:30 PM - 9:30 PM",
                            Image = "https://images.tkbcdn.com/1/1560/600/Upload/eventcover/2023/07/23/B8A309.jpg",
                            IsDeleted = false,
                            Location = "209 Đường 30 Tháng 4, Xuân Khánh, Ninh Kiều, TP. Cần Thơ",
                            MinPrice = 350000,
                            Status = 0,
                            Title = "[MÂY LANG THANG CẦN THƠ] LIVESHOW ƯNG HOÀNG PHÚC & PHẠM QUỲNH ANH"
                        },
                        new
                        {
                            Id = "e5",
                            City = "",
                            Date = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Duration = "6:30 PM - 7:00 PM",
                            Image = "https://images.tkbcdn.com/1/1560/600/Upload/eventcover/2023/07/23/46B510.jpg",
                            IsDeleted = false,
                            Location = "Bờ Đông cầu Rồng, đường Trần Hưng Đạo, quận Sơn Trà, Đà Nẵng",
                            MinPrice = 500000,
                            Status = 0,
                            Title = "[MÂY LANG THANG ĐÀ NẴNG] LIVESHOW TRUNG QUÂN IDOL"
                        },
                        new
                        {
                            Id = "e6",
                            City = "",
                            Date = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Duration = "7:30 PM - 9:30 PM",
                            Image = "https://images.tkbcdn.com/1/1560/600/Upload/eventcover/2023/07/10/AA7A83.jpg",
                            IsDeleted = false,
                            Location = "NHÀ HÁT GIAO HƯỞNG NHẠC VŨ KỊCH TP. HỒ CHÍ MINH",
                            MinPrice = 450000,
                            Status = 0,
                            Title = "[HBSO] ROCK SYMPHONY VOL.3"
                        });
                });

            modelBuilder.Entity("TicketBooking.API.Models.EventCategory", b =>
                {
                    b.Property<string>("EventId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("CategoryId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("EventId", "CategoryId");

                    b.HasIndex("CategoryId");

                    b.ToTable("EventCategory", (string)null);

                    b.HasData(
                        new
                        {
                            EventId = "e1",
                            CategoryId = "c1"
                        },
                        new
                        {
                            EventId = "e2",
                            CategoryId = "c1"
                        },
                        new
                        {
                            EventId = "e3",
                            CategoryId = "c1"
                        },
                        new
                        {
                            EventId = "e4",
                            CategoryId = "c3"
                        },
                        new
                        {
                            EventId = "e5",
                            CategoryId = "c2"
                        },
                        new
                        {
                            EventId = "e6",
                            CategoryId = "c2"
                        },
                        new
                        {
                            EventId = "e5",
                            CategoryId = "c4"
                        });
                });

            modelBuilder.Entity("TicketBooking.API.Models.Invoice", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("nvarchar(450)")
                        .HasDefaultValue("c787e20b-bf82-4f96-a7e8-7d1c7e156bfe");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValue(new DateTime(2023, 7, 24, 14, 58, 8, 902, DateTimeKind.Local).AddTicks(6317));

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("EventId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<string>("Mail")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("EventId");

                    b.ToTable("Invoice", (string)null);
                });

            modelBuilder.Entity("TicketBooking.API.Models.Seat", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("nvarchar(450)")
                        .HasDefaultValue("cdd519ad-01cb-44b9-8206-17e30a45affd");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValue(new DateTime(2023, 7, 24, 14, 58, 8, 904, DateTimeKind.Local).AddTicks(4800));

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Type")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.ToTable("Seat", (string)null);

                    b.HasData(
                        new
                        {
                            Id = "A1",
                            IsDeleted = false,
                            Name = "A1",
                            Type = 0
                        },
                        new
                        {
                            Id = "A2",
                            IsDeleted = false,
                            Name = "A2",
                            Type = 0
                        },
                        new
                        {
                            Id = "A3",
                            IsDeleted = false,
                            Name = "A3",
                            Type = 0
                        },
                        new
                        {
                            Id = "A4",
                            IsDeleted = false,
                            Name = "A4",
                            Type = 0
                        },
                        new
                        {
                            Id = "A5",
                            IsDeleted = false,
                            Name = "A5",
                            Type = 0
                        },
                        new
                        {
                            Id = "A6",
                            IsDeleted = false,
                            Name = "A6",
                            Type = 0
                        },
                        new
                        {
                            Id = "A7",
                            IsDeleted = false,
                            Name = "A7",
                            Type = 0
                        },
                        new
                        {
                            Id = "A8",
                            IsDeleted = false,
                            Name = "A8",
                            Type = 0
                        },
                        new
                        {
                            Id = "A9",
                            IsDeleted = false,
                            Name = "A9",
                            Type = 0
                        },
                        new
                        {
                            Id = "A10",
                            IsDeleted = false,
                            Name = "A10",
                            Type = 0
                        },
                        new
                        {
                            Id = "A11",
                            IsDeleted = false,
                            Name = "A11",
                            Type = 0
                        },
                        new
                        {
                            Id = "A12",
                            IsDeleted = false,
                            Name = "A12",
                            Type = 0
                        },
                        new
                        {
                            Id = "A13",
                            IsDeleted = false,
                            Name = "A13",
                            Type = 0
                        },
                        new
                        {
                            Id = "A14",
                            IsDeleted = false,
                            Name = "A14",
                            Type = 0
                        },
                        new
                        {
                            Id = "A15",
                            IsDeleted = false,
                            Name = "A15",
                            Type = 0
                        },
                        new
                        {
                            Id = "A16",
                            IsDeleted = false,
                            Name = "A16",
                            Type = 0
                        },
                        new
                        {
                            Id = "A17",
                            IsDeleted = false,
                            Name = "A17",
                            Type = 0
                        },
                        new
                        {
                            Id = "A18",
                            IsDeleted = false,
                            Name = "A18",
                            Type = 0
                        },
                        new
                        {
                            Id = "B1",
                            IsDeleted = false,
                            Name = "B1",
                            Type = 0
                        },
                        new
                        {
                            Id = "B2",
                            IsDeleted = false,
                            Name = "B2",
                            Type = 0
                        },
                        new
                        {
                            Id = "B3",
                            IsDeleted = false,
                            Name = "B3",
                            Type = 0
                        },
                        new
                        {
                            Id = "B4",
                            IsDeleted = false,
                            Name = "B4",
                            Type = 0
                        },
                        new
                        {
                            Id = "B5",
                            IsDeleted = false,
                            Name = "B5",
                            Type = 0
                        },
                        new
                        {
                            Id = "B6",
                            IsDeleted = false,
                            Name = "B6",
                            Type = 0
                        },
                        new
                        {
                            Id = "B7",
                            IsDeleted = false,
                            Name = "B7",
                            Type = 0
                        },
                        new
                        {
                            Id = "B8",
                            IsDeleted = false,
                            Name = "B8",
                            Type = 0
                        },
                        new
                        {
                            Id = "B9",
                            IsDeleted = false,
                            Name = "B9",
                            Type = 0
                        },
                        new
                        {
                            Id = "B10",
                            IsDeleted = false,
                            Name = "B10",
                            Type = 0
                        },
                        new
                        {
                            Id = "B11",
                            IsDeleted = false,
                            Name = "B11",
                            Type = 0
                        },
                        new
                        {
                            Id = "B12",
                            IsDeleted = false,
                            Name = "B12",
                            Type = 0
                        },
                        new
                        {
                            Id = "B13",
                            IsDeleted = false,
                            Name = "B13",
                            Type = 0
                        },
                        new
                        {
                            Id = "B14",
                            IsDeleted = false,
                            Name = "B14",
                            Type = 0
                        },
                        new
                        {
                            Id = "B15",
                            IsDeleted = false,
                            Name = "B15",
                            Type = 0
                        },
                        new
                        {
                            Id = "B16",
                            IsDeleted = false,
                            Name = "B16",
                            Type = 0
                        },
                        new
                        {
                            Id = "B17",
                            IsDeleted = false,
                            Name = "B17",
                            Type = 0
                        },
                        new
                        {
                            Id = "B18",
                            IsDeleted = false,
                            Name = "B18",
                            Type = 0
                        },
                        new
                        {
                            Id = "C1",
                            IsDeleted = false,
                            Name = "C1",
                            Type = 1
                        },
                        new
                        {
                            Id = "C2",
                            IsDeleted = false,
                            Name = "C2",
                            Type = 1
                        },
                        new
                        {
                            Id = "C3",
                            IsDeleted = false,
                            Name = "C3",
                            Type = 1
                        },
                        new
                        {
                            Id = "C4",
                            IsDeleted = false,
                            Name = "C4",
                            Type = 1
                        },
                        new
                        {
                            Id = "C5",
                            IsDeleted = false,
                            Name = "C5",
                            Type = 1
                        },
                        new
                        {
                            Id = "C6",
                            IsDeleted = false,
                            Name = "C6",
                            Type = 1
                        },
                        new
                        {
                            Id = "C7",
                            IsDeleted = false,
                            Name = "C7",
                            Type = 1
                        },
                        new
                        {
                            Id = "C8",
                            IsDeleted = false,
                            Name = "C8",
                            Type = 1
                        },
                        new
                        {
                            Id = "C9",
                            IsDeleted = false,
                            Name = "C9",
                            Type = 1
                        },
                        new
                        {
                            Id = "C10",
                            IsDeleted = false,
                            Name = "C10",
                            Type = 1
                        },
                        new
                        {
                            Id = "C11",
                            IsDeleted = false,
                            Name = "C11",
                            Type = 1
                        },
                        new
                        {
                            Id = "C12",
                            IsDeleted = false,
                            Name = "C12",
                            Type = 1
                        },
                        new
                        {
                            Id = "C13",
                            IsDeleted = false,
                            Name = "C13",
                            Type = 1
                        },
                        new
                        {
                            Id = "C14",
                            IsDeleted = false,
                            Name = "C14",
                            Type = 1
                        },
                        new
                        {
                            Id = "C15",
                            IsDeleted = false,
                            Name = "C15",
                            Type = 1
                        },
                        new
                        {
                            Id = "C16",
                            IsDeleted = false,
                            Name = "C16",
                            Type = 1
                        },
                        new
                        {
                            Id = "C17",
                            IsDeleted = false,
                            Name = "C17",
                            Type = 1
                        },
                        new
                        {
                            Id = "C18",
                            IsDeleted = false,
                            Name = "C18",
                            Type = 1
                        },
                        new
                        {
                            Id = "D1",
                            IsDeleted = false,
                            Name = "D1",
                            Type = 1
                        },
                        new
                        {
                            Id = "D2",
                            IsDeleted = false,
                            Name = "D2",
                            Type = 1
                        },
                        new
                        {
                            Id = "D3",
                            IsDeleted = false,
                            Name = "D3",
                            Type = 1
                        },
                        new
                        {
                            Id = "D4",
                            IsDeleted = false,
                            Name = "D4",
                            Type = 1
                        },
                        new
                        {
                            Id = "D5",
                            IsDeleted = false,
                            Name = "D5",
                            Type = 1
                        },
                        new
                        {
                            Id = "D6",
                            IsDeleted = false,
                            Name = "D6",
                            Type = 1
                        },
                        new
                        {
                            Id = "D7",
                            IsDeleted = false,
                            Name = "D7",
                            Type = 1
                        },
                        new
                        {
                            Id = "D8",
                            IsDeleted = false,
                            Name = "D8",
                            Type = 1
                        },
                        new
                        {
                            Id = "D9",
                            IsDeleted = false,
                            Name = "D9",
                            Type = 1
                        },
                        new
                        {
                            Id = "D10",
                            IsDeleted = false,
                            Name = "D10",
                            Type = 1
                        },
                        new
                        {
                            Id = "D11",
                            IsDeleted = false,
                            Name = "D11",
                            Type = 1
                        },
                        new
                        {
                            Id = "D12",
                            IsDeleted = false,
                            Name = "D12",
                            Type = 1
                        },
                        new
                        {
                            Id = "D13",
                            IsDeleted = false,
                            Name = "D13",
                            Type = 1
                        },
                        new
                        {
                            Id = "D14",
                            IsDeleted = false,
                            Name = "D14",
                            Type = 1
                        },
                        new
                        {
                            Id = "D15",
                            IsDeleted = false,
                            Name = "D15",
                            Type = 1
                        },
                        new
                        {
                            Id = "D16",
                            IsDeleted = false,
                            Name = "D16",
                            Type = 1
                        },
                        new
                        {
                            Id = "D17",
                            IsDeleted = false,
                            Name = "D17",
                            Type = 1
                        },
                        new
                        {
                            Id = "D18",
                            IsDeleted = false,
                            Name = "D18",
                            Type = 1
                        },
                        new
                        {
                            Id = "E1",
                            IsDeleted = false,
                            Name = "E1",
                            Type = 2
                        },
                        new
                        {
                            Id = "E2",
                            IsDeleted = false,
                            Name = "E2",
                            Type = 2
                        },
                        new
                        {
                            Id = "E3",
                            IsDeleted = false,
                            Name = "E3",
                            Type = 2
                        },
                        new
                        {
                            Id = "E4",
                            IsDeleted = false,
                            Name = "E4",
                            Type = 2
                        },
                        new
                        {
                            Id = "E5",
                            IsDeleted = false,
                            Name = "E5",
                            Type = 2
                        },
                        new
                        {
                            Id = "E6",
                            IsDeleted = false,
                            Name = "E6",
                            Type = 2
                        },
                        new
                        {
                            Id = "E7",
                            IsDeleted = false,
                            Name = "E7",
                            Type = 2
                        },
                        new
                        {
                            Id = "E8",
                            IsDeleted = false,
                            Name = "E8",
                            Type = 2
                        },
                        new
                        {
                            Id = "E9",
                            IsDeleted = false,
                            Name = "E9",
                            Type = 2
                        },
                        new
                        {
                            Id = "E10",
                            IsDeleted = false,
                            Name = "E10",
                            Type = 2
                        },
                        new
                        {
                            Id = "E11",
                            IsDeleted = false,
                            Name = "E11",
                            Type = 2
                        },
                        new
                        {
                            Id = "E12",
                            IsDeleted = false,
                            Name = "E12",
                            Type = 2
                        },
                        new
                        {
                            Id = "E13",
                            IsDeleted = false,
                            Name = "E13",
                            Type = 2
                        },
                        new
                        {
                            Id = "E14",
                            IsDeleted = false,
                            Name = "E14",
                            Type = 2
                        },
                        new
                        {
                            Id = "E15",
                            IsDeleted = false,
                            Name = "E15",
                            Type = 2
                        },
                        new
                        {
                            Id = "E16",
                            IsDeleted = false,
                            Name = "E16",
                            Type = 2
                        },
                        new
                        {
                            Id = "E17",
                            IsDeleted = false,
                            Name = "E17",
                            Type = 2
                        },
                        new
                        {
                            Id = "E18",
                            IsDeleted = false,
                            Name = "E18",
                            Type = 2
                        },
                        new
                        {
                            Id = "F1",
                            IsDeleted = false,
                            Name = "F1",
                            Type = 2
                        },
                        new
                        {
                            Id = "F2",
                            IsDeleted = false,
                            Name = "F2",
                            Type = 2
                        },
                        new
                        {
                            Id = "F3",
                            IsDeleted = false,
                            Name = "F3",
                            Type = 2
                        },
                        new
                        {
                            Id = "F4",
                            IsDeleted = false,
                            Name = "F4",
                            Type = 2
                        },
                        new
                        {
                            Id = "F5",
                            IsDeleted = false,
                            Name = "F5",
                            Type = 2
                        },
                        new
                        {
                            Id = "F6",
                            IsDeleted = false,
                            Name = "F6",
                            Type = 2
                        },
                        new
                        {
                            Id = "F7",
                            IsDeleted = false,
                            Name = "F7",
                            Type = 2
                        },
                        new
                        {
                            Id = "F8",
                            IsDeleted = false,
                            Name = "F8",
                            Type = 2
                        },
                        new
                        {
                            Id = "F9",
                            IsDeleted = false,
                            Name = "F9",
                            Type = 2
                        },
                        new
                        {
                            Id = "F10",
                            IsDeleted = false,
                            Name = "F10",
                            Type = 2
                        },
                        new
                        {
                            Id = "F11",
                            IsDeleted = false,
                            Name = "F11",
                            Type = 2
                        },
                        new
                        {
                            Id = "F12",
                            IsDeleted = false,
                            Name = "F12",
                            Type = 2
                        },
                        new
                        {
                            Id = "F13",
                            IsDeleted = false,
                            Name = "F13",
                            Type = 2
                        },
                        new
                        {
                            Id = "F14",
                            IsDeleted = false,
                            Name = "F14",
                            Type = 2
                        },
                        new
                        {
                            Id = "F15",
                            IsDeleted = false,
                            Name = "F15",
                            Type = 2
                        },
                        new
                        {
                            Id = "F16",
                            IsDeleted = false,
                            Name = "F16",
                            Type = 2
                        },
                        new
                        {
                            Id = "F17",
                            IsDeleted = false,
                            Name = "F17",
                            Type = 2
                        },
                        new
                        {
                            Id = "F18",
                            IsDeleted = false,
                            Name = "F18",
                            Type = 2
                        });
                });

            modelBuilder.Entity("TicketBooking.API.Models.SeatEvent", b =>
                {
                    b.Property<string>("EventId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("SeatId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("Price")
                        .HasColumnType("int");

                    b.Property<int>("SeatStatus")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.HasKey("EventId", "SeatId");

                    b.HasIndex("SeatId");

                    b.ToTable("SeatEvent", (string)null);

                    b.HasData(
                        new
                        {
                            EventId = "e1",
                            SeatId = "A1",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A2",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A3",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A4",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A5",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A6",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A7",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A8",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A9",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A10",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A11",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A12",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A13",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A14",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A15",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A16",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A17",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "A18",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B1",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B2",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B3",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B4",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B5",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B6",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B7",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B8",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B9",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B10",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B11",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B12",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B13",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B14",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B15",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B16",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B17",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "B18",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C1",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C2",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C3",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C4",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C5",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C6",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C7",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C8",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C9",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C10",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C11",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C12",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C13",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C14",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C15",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C16",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C17",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "C18",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D1",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D2",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D3",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D4",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D5",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D6",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D7",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D8",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D9",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D10",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D11",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D12",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D13",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D14",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D15",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D16",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D17",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "D18",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E1",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E2",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E3",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E4",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E5",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E6",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E7",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E8",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E9",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E10",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E11",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E12",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E13",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E14",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E15",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E16",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E17",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "E18",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F1",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F2",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F3",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F4",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F5",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F6",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F7",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F8",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F9",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F10",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F11",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F12",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F13",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F14",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F15",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F16",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F17",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e1",
                            SeatId = "F18",
                            Price = 60000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A1",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A2",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A3",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A4",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A5",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A6",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A7",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A8",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A9",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A10",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A11",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A12",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A13",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A14",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A15",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A16",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A17",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "A18",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B1",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B2",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B3",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B4",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B5",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B6",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B7",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B8",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B9",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B10",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B11",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B12",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B13",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B14",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B15",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B16",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B17",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "B18",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C1",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C2",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C3",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C4",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C5",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C6",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C7",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C8",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C9",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C10",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C11",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C12",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C13",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C14",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C15",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C16",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C17",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "C18",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D1",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D2",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D3",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D4",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D5",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D6",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D7",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D8",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D9",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D10",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D11",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D12",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D13",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D14",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D15",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D16",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D17",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "D18",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E1",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E2",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E3",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E4",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E5",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E6",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E7",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E8",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E9",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E10",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E11",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E12",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E13",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E14",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E15",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E16",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E17",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "E18",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F1",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F2",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F3",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F4",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F5",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F6",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F7",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F8",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F9",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F10",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F11",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F12",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F13",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F14",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F15",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F16",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F17",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e2",
                            SeatId = "F18",
                            Price = 70000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "A18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "B18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "C18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "D18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "E18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e3",
                            SeatId = "F18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "A18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "B18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "C18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "D18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "E18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e4",
                            SeatId = "F18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "A18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "B18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "C18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "D18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "E18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e5",
                            SeatId = "F18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "A18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "B18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "C18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "D18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "E18",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F1",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F2",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F3",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F4",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F5",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F6",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F7",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F8",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F9",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F10",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F11",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F12",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F13",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F14",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F15",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F16",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F17",
                            Price = 100000,
                            SeatStatus = 0
                        },
                        new
                        {
                            EventId = "e6",
                            SeatId = "F18",
                            Price = 100000,
                            SeatStatus = 0
                        });
                });

            modelBuilder.Entity("TicketBooking.API.Models.SeatInvoice", b =>
                {
                    b.Property<string>("InvoiceId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("SeatId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("InvoiceId", "SeatId");

                    b.HasIndex("SeatId");

                    b.ToTable("SeatInvoice", (string)null);
                });

            modelBuilder.Entity("TicketBooking.API.Models.EventCategory", b =>
                {
                    b.HasOne("TicketBooking.API.Models.Category", null)
                        .WithMany()
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("TicketBooking.API.Models.Event", null)
                        .WithMany()
                        .HasForeignKey("EventId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("TicketBooking.API.Models.Invoice", b =>
                {
                    b.HasOne("TicketBooking.API.Models.Event", "Event")
                        .WithMany("Invoices")
                        .HasForeignKey("EventId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Event");
                });

            modelBuilder.Entity("TicketBooking.API.Models.SeatEvent", b =>
                {
                    b.HasOne("TicketBooking.API.Models.Event", "Event")
                        .WithMany("SeatEvents")
                        .HasForeignKey("EventId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("TicketBooking.API.Models.Seat", "Seat")
                        .WithMany("SeatEvents")
                        .HasForeignKey("SeatId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Event");

                    b.Navigation("Seat");
                });

            modelBuilder.Entity("TicketBooking.API.Models.SeatInvoice", b =>
                {
                    b.HasOne("TicketBooking.API.Models.Invoice", null)
                        .WithMany()
                        .HasForeignKey("InvoiceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("TicketBooking.API.Models.Seat", null)
                        .WithMany()
                        .HasForeignKey("SeatId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("TicketBooking.API.Models.Event", b =>
                {
                    b.Navigation("Invoices");

                    b.Navigation("SeatEvents");
                });

            modelBuilder.Entity("TicketBooking.API.Models.Seat", b =>
                {
                    b.Navigation("SeatEvents");
                });
#pragma warning restore 612, 618
        }
    }
}
